...
document::document(const std::string &path) {
    ...
    for (int i = 0; i < pageCount; ++i) {
        ...
        page_acc_h.emplace_back(
                page_acc_h.back()+
                fz_bound_page(ctx, pages.at(i)).y1);
        if(fz_bound_page(ctx, pages.at(i)).x1>max_page_width){

            // Note: Here i made an assumption that (x0,y0) of pdf file are always (0,0)
            // thus x1 value = width. Is this always true??
